...
def _format_section(title, items):
    """Format strings or grouped rules into readable prompt sections."""
    ...
    if isinstance(items, dict):
        ...
        for name, rules in items.items():
            ...
            if not (separator and prefix.isdigit()):
                label = name
            heading = label.replace("_", " ").upper()
            sections.append(f"### {heading} ###")
            sections.append(_format_section(heading, rules))
        return "\n".join(sections)

    return str(items)

def build_system_prompt(identity, memory, action_guide):
